...
- **`lessons/`** — the Pavilion's own **followable lessons** (the school's content home; the Course Board/Academy draw on these). The first, [`lessons/01-a-beating-heart.md`](lessons/01-a-beating-heart.md), takes a newcomer from nothing to a running local AI — including checking what their machine can run. Roadmap: `plans/COURSE-PROGRESSION-PLAN.md`.
