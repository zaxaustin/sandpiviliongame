# The Sand Pavilion — A Visitor's Handbook

This is the user manual: how to actually *live* in the Pavilion once it's
running — every room, every desk, every resident, and where your things are
kept. It assumes nothing technical. Getting the app running in the first
place is the [README](README.md)'s job ("Running it — every mode"); filling
your Pavilion with books and your own AI is [`PROTOCOLS.md`](PROTOCOLS.md)'s
job. This handbook is the layer between: *what everything is and how to use it.*

Two promises, up front, that make the rest easy to relax into:

- **Nothing breaks by clicking around.** Every panel closes with Esc or the
  ✕ in its corner. There is no wrong door.
- **Nothing phones home.** Everything you write, read, and plan saves to
  this device. The only network the Pavilion touches is the one you
  deliberately give it (your own local AI, or a cloud key you add yourself).

---

## 1. The controls, all of them

| Key | What it does |
| --- | --- |
| **Arrows / WASD** | Walk |
| **E** (or Enter) | Talk, read, open a desk, fish — "interact with what I'm facing" |
| **Esc** | Close whatever's open; with nothing open, the pause menu |
...
## 4. The Library — reading, notes, and Your Shelf
...
The HUD's due-badge and the **Upcoming** view read one spine: planner
due-dates, course steps, and calendar events together — one honest picture
of the day, not three.

---

## 6. The Workshop — Sebastian, the calendar, and the working desks

- **Sebastian's Calendar** (the red-banded desk pad by his desk) — a month
  grid and day view. At its top sits his **advisory read** of the day
...
## 7. Making the residents talk — the one real setup step

Everything above works with zero setup. The residents' actual
conversations run on an AI that lives **on your computer** — free, private,
and yours; that's the point, and it's the one piece with real steps:

1. Install [Ollama](https://ollama.com) and pull a model that fits your
   machine — the tiered picking guide is in [`PROTOCOLS.md`](PROTOCOLS.md)
   Protocol 2 (and inside the game: ⚙ Manage AI connections → 📏 *Which
   model fits my machine?*). When in doubt: `ollama pull llama3.2`.
...
## 8. Your data — where everything lives

- **📊 Your Data** (pause menu) — what's actually in your save: real size,
  counts, and pruning for old planner days.
- **⬇ Export save / ⬆ Import save** — your whole Pavilion as one `.json`
...
*The deeper layers, when you want them: [`PROTOCOLS.md`](PROTOCOLS.md) for
growing your own Pavilion, [`LEARNING-PATH.md`](LEARNING-PATH.md) for
learning how any of this actually works, built around this very project —
and the shelved book "How to Complete Your Own Pavilion" (the Library's
Practice shelf) for the in-world telling.*
